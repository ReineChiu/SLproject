...
connection_pool = pooling.MySQLConnectionPool(pool_name = ,
                                              pool_size = 5,
                                              host = ,
                                              user = ,
                                              database = ,  
...
def select_all_player(*args):
    try:
        connection_object = connection_pool.get_connection()
        cursor = connection_object.cursor(dictionary=True, buffered=True)
        select = "select * from standings_all_player where player_name=%s and birthday=%s" 
...
def update_allplayer(*args):
    try: 
        connection_object = connection_pool.get_connection()
        cursor = connection_object.cursor()

        update = ("update standings_all_player set army=%s,num=%s,pos=%s,habits=%s,o_name=%s,retire=%s,team_id=%s where id=%s")
        cursor.execute(update, args)
        connection_object.commit()
    except Exception as e:
        print(f'{e}:更新總表球員資料失敗')
    finally:
        cursor.close()
        connection_object.close()
...
def select_game(_id):
    try:
        connection_object = connection_pool.get_connection()
        cursor = connection_object.cursor(dictionary=True, buffered=True)
        select = "select * from standings_game where id=%s"
        value = [_id]
        cursor.execute(select, value)
        result = cursor.fetchone()
        return result
...
def update_game(*args):
    try: 
        ...
        update = ("update standings_game set homeScore=%s,homePitch=%s,guestScore=%s,guestPitch=%s,winPitch=%s,losePitch=%s,mvp=%s where id=%s")
